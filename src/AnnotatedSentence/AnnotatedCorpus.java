...
public class AnnotatedCorpus extends Corpus{
    ...
    public void automaticMorphologicalDisambiguation(){
        FsmMorphologicalAnalyzer fsm = new FsmMorphologicalAnalyzer();
        HmmDisambiguation hmmDisambiguator = new HmmDisambiguation();
        hmmDisambiguator.loadModel();
        for (int i = 0; i < sentenceCount(); i++){
            AnnotatedSentence sentence = (AnnotatedSentence) getSentence(i);
            boolean changed = false;
            FsmParseList[] fsmParses = fsm.robustMorphologicalAnalysis(sentence);
            ArrayList<FsmParse> correctParses = hmmDisambiguator.disambiguate(fsmParses);
            if (correctParses != null && correctParses.size() == sentence.wordCount()){
                int index = 0;
                for (FsmParse parse : correctParses){
                    AnnotatedWord word = (AnnotatedWord) sentence.getWord(index);
                    if (word.getParse() == null){
                        word.setParse(parse.transitionList());
                        word.setMetamorphicParse(parse.withList());
                        changed = true;
                    }
                    index++;
                }
    ...
    public void automaticSingleSemantic(){
        FsmMorphologicalAnalyzer fsm = new FsmMorphologicalAnalyzer();
        ...
        for (int i = 0; i < sentenceCount(); i++){
            ...
            for (Word word : sentence.getWords()){
                AnnotatedWord current = (AnnotatedWord) word;
                if (current.getParse() != null){
                    ArrayList<SynSet> meanings = turkish.constructSynSets(current.getParse().getWord().getName(), current.getParse(), current.getMetamorphicParse(), fsm);
                    if (meanings.size() == 1){
                        ((AnnotatedWord) word).setSemantic(meanings.get(0).getId());
                        changed = true;
                    }
                }
            }
    ...
    public void automaticSingleMorphologicalAnalysis(){
        FsmMorphologicalAnalyzer fsm = new FsmMorphologicalAnalyzer();
        for (int i = 0; i < sentenceCount(); i++){
            AnnotatedSentence sentence = (AnnotatedSentence) getSentence(i);
            boolean changed = false;
            for (Word word : sentence.getWords()){
                AnnotatedWord current = (AnnotatedWord) word;
                if (word.getName() != null && current.getParse() == null){
                    FsmParseList parseList = fsm.morphologicalAnalysis(current.getName());
                    if (parseList.size() == 1){
                        ((AnnotatedWord) word).setParse(parseList.getFsmParse(0).transitionList());
                        ((AnnotatedWord) word).setMetamorphicParse(parseList.getFsmParse(0).withList());
                        changed = true;
                    }
                }
            }
